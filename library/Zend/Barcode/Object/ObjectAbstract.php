...
abstract class Zend_Barcode_Object_ObjectAbstract {
	...
	protected function _addPolygon(array $points, $color = null, $filled = true) {
		...
		$this->_addInstruction ( array ('type' => 'polygon', 'points' => $points, 'color' => $color, 'filled' => $filled ) );
	}
	...
	protected function _addText($text, $size, $position, $font, $color, $alignment = 'center', $orientation = 0) {
		...
		$this->_addInstruction ( array ('type' => 'text', 'text' => $text, 'size' => $size, 'position' => $position, 'font' => $font, 'color' => $color, 'alignment' => $alignment, 'orientation' => $orientation ) );
	}
	...
	public function getOffsetLeft($recalculate = false) {
		if ($this->_offsetLeft === null || $recalculate) {
			$this->_offsetLeft = - min ( array (0 * cos ( $this->_orientation / 180 * pi () ) - 0 * sin ( $this->_orientation / 180 * pi () ), 0 * cos ( $this->_orientation / 180 * pi () ) - $this->_calculateBarcodeHeight () * sin ( $this->_orientation / 180 * pi () ), $this->_calculateBarcodeWidth () * cos ( $this->_orientation / 180 * pi () ) - $this->_calculateBarcodeHeight () * sin ( $this->_orientation / 180 * pi () ), $this->_calculateBarcodeWidth () * cos ( $this->_orientation / 180 * pi () ) - 0 * sin ( $this->_orientation / 180 * pi () ) ) );
		}
	...
	public function getOffsetTop($recalculate = false) {
		if ($this->_offsetTop === null || $recalculate) {
			$this->_offsetTop = - min ( array (0 * cos ( $this->_orientation / 180 * pi () ) + 0 * sin ( $this->_orientation / 180 * pi () ), $this->_calculateBarcodeHeight () * cos ( $this->_orientation / 180 * pi () ) + 0 * sin ( $this->_orientation / 180 * pi () ), $this->_calculateBarcodeHeight () * cos ( $this->_orientation / 180 * pi () ) + $this->_calculateBarcodeWidth () * sin ( $this->_orientation / 180 * pi () ), 0 * cos ( $this->_orientation / 180 * pi () ) + $this->_calculateBarcodeWidth () * sin ( $this->_orientation / 180 * pi () ) ) );
		}
	...
	protected function _drawBarcode() {
		...
		$this->_addPolygon ( array ($point1, $point2, $point3, $point4 ), $this->_backgroundColor );
		...
		foreach ( $barcodeTable as $bar ) {
			...
			if ($bar [0]) {
				...
				$this->_addPolygon ( array ($point1, $point2, $point3, $point4 ) );
			}
			$xpos += $width;
		}
	...
	protected function _drawBorder() {
		if ($this->_withBorder) {
			...
			$this->_addPolygon ( array ($point1, $point2, $point3, $point4, $point1 ), $this->_foreColor, false );
		}
	}
