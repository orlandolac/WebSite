...
class Zend_Barcode_Renderer_Svg extends Zend_Barcode_Renderer_RendererAbstract {
	...
	protected function _initRenderer() {
		...
		$imageBackgroundColor = 'rgb(' . implode ( ', ', array (($backgroundColor & 0xFF0000) >> 16, ($backgroundColor & 0x00FF00) >> 8, ($backgroundColor & 0x0000FF) ) ) . ')';
	...
	protected function _drawPolygon($points, $color, $filled = true) {
		$color = 'rgb(' . implode ( ', ', array (($color & 0xFF0000) >> 16, ($color & 0x00FF00) >> 8, ($color & 0x0000FF) ) ) . ')';
		$orientation = $this->getBarcode ()->getOrientation ();
		$newPoints = array ($points [0] [0] + $this->_leftOffset, $points [0] [1] + $this->_topOffset, $points [1] [0] + $this->_leftOffset, $points [1] [1] + $this->_topOffset, $points [2] [0] + $this->_leftOffset + cos ( - $orientation ), $points [2] [1] + $this->_topOffset - sin ( $orientation ), $points [3] [0] + $this->_leftOffset + cos ( - $orientation ), $points [3] [1] + $this->_topOffset - sin ( $orientation ) );
