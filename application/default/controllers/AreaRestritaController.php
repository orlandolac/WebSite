...
/**
 * Controller da área restrita do sistema.
 * 
 * @category    Controller
 ...
 * @version     1.14.04.05
 */
class Default_AreaRestritaController extends Pu_Controller_Action {

    public function init() {
        parent::init();
        $this->trilha[] = '<a href="/area-restrita">Área restrita</a>';
        $action = $this->getRequest()->getParam('action');
        if(!in_array($action, array('sair', 'entrar-facebook', 'entrar-google', 'entrar-twitter', 'entrar-linkedin'))){
            if (Zend_Auth::getInstance()->hasIdentity()) {
                Pu_Alert::add('Você já está cadastrado e autenticado!');
                $this->_redirect('/');
            }
        }
    }
    ...
    public function entrarAction() {
        if ($this->getRequest()->isPost()) {
            if(!$this->entrar($this->getRequest()->getPost())){
                $this->_redirect('/area-restrita');
            }
        }
        $this->_redirect('/');
    ...
    public function entrarFacebookAction(){
        require SYS_PATH . '/library/Facebook/facebook.php';
        $facebook = new Facebook(array(
            'appId'  => SYS_SOCIAL_FB_APP_ID,
            'secret' => SYS_SOCIAL_FB_SECRET,
            'fileUpload' => true,
        ));
        
        if(!($user_id = $facebook->getUser())) {
            Pu_Alert::add('Usuário não identificado.', Pu_Alert::$DANGER);
            $this->_redirect('/');
        }
        ...
        try {
            ...
            Pu_Alert::add('Não foi possível recuperar os dados de sua conta.', Pu_Alert::$DANGER);
            $this->_redirect('/');
        }
        
        $model = new Default_Model_Usuario();
        if(($user = $model->getByFacebookId($data['id']))){
            $model->updateUserFacebook($user, $data);
            $this->entrar($user, 'Facebook');
        }else{
            if(($user = $model->getByEMail($data['email']))){
                if($user['user_id_facebook'] > 0){
                    Pu_Alert::add('Este e-mail já está sendo utilizado por outro usuário.', Pu_Alert::$DANGER);
                    Pu_Alert::add('Se este e-mail lhe pertence entre em contato conosco para solucionarmos o problema.', Pu_Alert::$WARNING);
                }else{
                    $model->updateUserFacebook($user, $data);
                    $this->entrar($user, 'Facebook');
                }
            }else{
                $senha = $model->getNovaSenha();
                $user = array(
                    'user_status_id'    => '1',
                    'user_tipo_id'      => '1',
                ...
                if($model->insert($user)){
                    ...
                    Pu_Alert::add('Não foi possível realizar a autenticação.', Pu_Alert::$DANGER);
                }
            }
        }
        $this->_redirect('/');
    ...
    public function entrarGoogleAction(){
        require_once SYS_PATH . '/library/Google/src/Google_Client.php';
        require_once SYS_PATH . '/library/Google/src/contrib/Google_Oauth2Service.php';
        
        $client = new Google_Client();
        $client->setApplicationName(SYS_NOME);
        $client->setClientId('728241411556.apps.googleusercontent.com');
        $client->setClientSecret('aVI3AWY07K6eOaSi1qPYd1MO');
        $client->setRedirectUri(SYS_URL . '/area-restrita/entrar-google');
    ...
    public function sairAction() {
        ...
        if (Zend_Auth::getInstance()->hasIdentity()) {
            ...
            Pu_Alert::add('Seu acesso foi encerrado corretamente !', Pu_Alert::$SUCCESS);
        }
        $this->_redirect('/');
    }
    ...
    public function entrar($get, $tpLogin='Tradicional') {
