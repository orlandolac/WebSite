 ...
 */
class Default_Model_Tabela_Perfil extends Zend_Db_Table_Abstract {
    ...
    public function buscar(array $get) {
        ...
        if(isset($get['palavra-chave']) && strlen($get['palavra-chave']) > 0){
            $where[] = 'user_nome like ' . $this->getDefaultAdapter()->quoteInto('?', '%' . $get['palavra-chave'] . '%');
        }
        ...
        $tmp = $this->_db->fetchRow('SELECT count(*) AS qtd FROM dfl_user ' . $SQL);
        if ($tmp['qtd'] > 0) {
            // PAGINA //
    ...
    public function detalhar($id){
        return $this->fetchRow($this->_db->quoteInto('user_id=?', $id));
    }
