 ...
 */
class Default_Model_Tabela_Ranking_Politicos extends Zend_Db_Table_Abstract {

    public $_db;
    protected $_name = 'pu_pessoa_x_ranking';
    protected $_primary = array('rank_ano', 'pessoa_id');
...
//        }
        ...
        $tmp = $this->_db->fetchRow('SELECT count(*) AS qtd FROM pu_pessoa_x_ranking R ' . $SQL);
        if ($tmp['qtd'] > 0) {
            // PAGINA //
            ...
            // BUSCA //
            if($SQL == ''){
                $SQL = ' WHERE R.pessoa_id=P.pessoa_id ';
            }else{
                $SQL = ' && R.pessoa_id=P.pessoa_id ';
            }
            
            $SQL = 'SELECT * FROM pu_pessoa_x_ranking R, pu_pessoa P ' . $SQL;
